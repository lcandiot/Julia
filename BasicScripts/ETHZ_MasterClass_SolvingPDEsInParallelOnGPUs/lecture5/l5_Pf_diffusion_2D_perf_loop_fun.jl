...
function Pf_diffusion_2D(; do_check=true, writeOut=true)
    ...
    # Write output
    if writeOut
        h5write("./data/Pf_diffusion_2D_perf_loop_fun.h5", "monitor/T_eff", T_eff)
        h5write("./data/Pf_diffusion_2D_perf_loop_fun.h5", "model/nx",      nx   )
        h5write("./data/Pf_diffusion_2D_perf_loop_fun.h5", "model/ny",      ny   ) 
    end
    # Return
    return
end
